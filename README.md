...
```bash
# 單元測試
cd skill-analyzer && go test ./... -v
cd skill-builder && go test ./... -v
...
- [實作計畫](.kiro/specs/skill-automation-pipeline/tasks.md)

## 規格和實作

本專案遵循規格驅動開發（Spec-Driven Development）方法論。完整的規格文檔位於 `.kiro/specs/skill-automation-pipeline/` 目錄：

- **requirements.md**: 20 個需求，涵蓋完整的工作流程
- **design.md**: 詳細的架構設計和 44 個正確性屬性
- **tasks.md**: 21 個主要任務的實作計畫

### 開始實作

規格已完成！你可以開始實作任務：

1. 打開 `.kiro/specs/skill-automation-pipeline/tasks.md` 查看完整的實作計畫
2. 任務按照依賴順序組織，包含檢查點以確保增量驗證
3. 每個任務都引用特定的需求以便追溯
4. 可選任務標記為 `*`，可以跳過以加快 MVP 開發

**測試策略：**
- 單元測試：驗證特定範例和邊界情況
- 屬性測試：使用 gopter 驗證通用正確性屬性
- 整合測試：驗證端到端工作流程
